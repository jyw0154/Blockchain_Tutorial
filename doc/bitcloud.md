...
* 블록 해시를 다 헤집지 않고도 이를 수월하게 하기 위해서는, 거래 내역은 머클트리(Merkle tree) 구조로 해시가 되며[7][2][5], 머클트리 구조의 루트 부분만 블록 해시에 포함되어야 함
...
---
## 8. 지불 입증 간소화
* 전체 네트워크 노드를 쓰지 않더라도, 돈이 지불된 사실을 입증하는 것이 가능
* 사용자는 가장 긴 작업증명 체인의 블록 헤더의 사본만 갖고 있으면, 자신이 가장 긴 체인이라 확일할 때까지 네트워크 노드들에게 요청하고, 그 거래 내역이 기록된 블록에 연결된 머클트리 가지만 받아오면 됨
* 그는 스스로 거래 내역을 확인할 수는 없고, 체인에 연결되어 네트워크 노드가 그것을 승인했는지, 이후에도 계속 블록이 추가로 확증되어 승인된 지로 알 수 있다.
